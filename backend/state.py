...
class State:
    ...
    def referee(self):
        ...
        empty = [] # 돌이 안두어져 있는 자리 리스트
        for i in range(15):
            for j in range(15):
                if self.black[i][j] == 0 and self.white[i][j] == 0:
                    empty.append(15 * i + j)

        if len(empty) == 0: # 꽉차있다면 비김
            end = 2
            return legal, illegal, end

        if not self.check_turn(): # 내 차례면 True, 아니면 False
            return empty, illegal, end

        for k in empty: # 돌이 안두어져 있는 자리 중에서 둘 수 있으면 legal, 둘 수 없으면 illegal에 append
            i, j = k // 15, k % 15
